...
% adoption of several scheduling policies. We consider the system does not
% satisfy Condition 1 in the paper. Hence, we consider the following 
% scheduling policies:
% 1. Indexed Priority policy  (IPP)
% 2. Benchmark Performance    (BP)
% 3. Greedy Policy    (GP)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [AoII_IPP,AoII_BP,AoII_GP] = run(N,w,p_pool,pe0_pool,pe1_pool, ...
                                          gamma_pool)
    ...
    while Rbar >= 1
        ...
        for j = 1:N
            ...
            pe0 = pe0_pool(j);
            gamma = gamma_pool(j);
            [n_est(:,j),~] = RVI(lambda_high,p,gamma,pe1,pe0, ...
                                 smax,epsilon,j,w);
            [~,rho(j)] = expected_value(p,c1,c2,n_est(:,j),gamma,smax,j,w);
        end
        Rbar = sum(rho);
...
function [AoII,rate] = expected_value(p,c1,c2,n,gamma,smax,j,w)
    if n(1) == smax + 2
        ...
        pi0_rev = 2 + p * (1-p)^(n1-1)*(1/(1-c1) - 1/p + ...
                  c1^(n0-n1)*(1/(1-c2)-1/(1-c1)));

        rate_tmp = p * (1-p)^(n1-1) * (gamma/(1-c1) + ...
                   c1^(n0-n1)*(1/(1-c2)-gamma/(1-c1)));
    ...
    elseif i ~= 1 && j == 1 % negative CSI
        if action == 2 % Transmit
            ret = (1-gamma)*(pe0*(1-p)+(1-pe0)*p)*RV_old(1,1) + ...
                  ...
                  RV_old(min(i+1,smax+1),1) + gamma*(pe0*(1-p) + ...
                  (1-pe0)*p)*RV_old(1,2) + gamma*(pe0*p+(1-pe0) * ...
                  (1-p))*RV_old(min(i+1,smax+1),2);
        else
        ...
        if action == 2
            ret = (1-gamma)*(pe1*p+(1-pe1)*(1-p))*RV_old(1,1) + ...
                  (1-gamma)*(pe1*(1-p)+(1-pe1)*p) * ...
                  RV_old(min(i+1,smax+1),1) + gamma * ...
                  (pe1*p+(1-pe1)*(1-p))*RV_old(1,2) + ... 
